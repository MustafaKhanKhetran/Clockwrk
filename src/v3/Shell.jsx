  ...
  return (
    <div className="v3-shell">
      ...
      <header className="v3-nav">
        ...
        {accountOpen && <aside className="v3-popover v3-account-menu"><div><Avatar name={me.name} /><span><strong>{me.name}</strong><small>{me.company}</small></span></div>{utility.map(([label, to, icon]) => <button key={to} onClick={() => navigate(to)}><Icon name={icon} size={16} />{label}<Icon name="arrow" size={14} /></button>)}</aside>}
      </header>

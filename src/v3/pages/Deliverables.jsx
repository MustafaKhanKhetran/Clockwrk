...
export default function Deliverables() {
  ...
  const files = useMemo(() => requests.flatMap((request) => request.deliverables.map((file) => ({ ...file, requestId: request.id, request: request.title, projectId: request.projectId, project: projects.find((item) => item.id === request.projectId) }))), [requests, projects]);
  ...
  return <div className="v3-files-page"><PageIntro index="Delivery archive" title="Deliverables" copy="Production files, source packages, live builds, and every version your team has shipped." />
    {files.length === 0 && <section className="v3-empty-panel v3-enter">
      <Icon name="folder" size={28} />
      <strong>No deliverables yet</strong>
      <span>The moment your team ships a file — a design, an export, a build — it lands here, filed under its request and project.</span>
    </section>}
    {files.length > 0 && <>
    ...
    <section className="v3-file-index v3-enter"><header><div><span>Delivery index</span><strong>{shown.length} {shown.length === 1 ? 'file' : 'files'}</strong></div><small>Every file stays attached to its request and project.</small></header><div>{shown.map((file) => <article key={file.id}><FileMark kind={file.kind} /><div className="v3-file-primary"><strong>{file.name}</strong><small>{file.size || 'Working file'} · version {file.version || 1}</small></div><div className="v3-file-context"><ProjectCode project={file.project} /><span><strong>{file.project?.name}</strong><small>{file.request}</small></span></div><div className="v3-file-meta"><span><Icon name="calendar" size={14} />{file.at}</span><span>{file.current ? 'Latest version' : 'Previous version'}</span></div>{file.url ? <a className="v3-file-download" href={file.url} target="_blank" rel="noreferrer" download aria-label={`Download ${file.name}`}><Icon name="download" size={17} /></a> : <button disabled aria-label="File not available yet"><Icon name="download" size={17} /></button>}</article>)}{!shown.length && <div className="v3-file-empty"><Icon name="search" size={22} /><strong>No matching files</strong><span>Try another project, format, or search term.</span></div>}</div></section>
    </>}
  </div>;
}

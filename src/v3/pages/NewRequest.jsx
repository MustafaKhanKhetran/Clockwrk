...
export default function NewRequest() {
  ...
  const onFilesChosen = async (event) => {
    ...
    for (const file of files) {
      try {
        ...
        setAttachments((current) => [...current, { url: r.url, name: r.name, size: r.size, mime: r.mime }]);
      } catch (err) {
  ...
  if (!projects.length) return <section className="v3-composer-success v3-empty-composer"><i><Icon name="projects" size={30} /></i><span>Project needed</span><h1>Give this work a home first.</h1><p>A project connects the brief to its team, files, decisions, and delivery history.</p><Action icon="plus" onClick={() => navigate('/projects/new')}>Create your first project</Action><button onClick={goBack}>Go back</button></section>;
  ...
  return <div className="v3-composer-page"><header><button onClick={goBack} aria-label="Back to previous page"><Icon name="close" /></button><span>New request</span><strong>{step + 1} / {steps.length}</strong></header><nav>{steps.map((item, index) => <button key={item} className={index === step ? 'is-active' : index < step ? 'is-done' : ''} onClick={() => index <= step && setStep(index)}><i>{index < step ? <Icon name="check" size={12} /> : index + 1}</i>{item}</button>)}</nav><main>
    ...
    {step === 2 && <section className="v3-composer-stage"><span>Write the brief</span><h1>Make the outcome unmistakable.</h1><p>Describe what success looks like, who it is for, and anything the team must keep.</p><div className="v3-brief-form"><label><span>Request title</span><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="e.g. Rebuild customer onboarding" /></label><label><span>What should the team make?</span><textarea value={brief} onChange={(event) => setBrief(event.target.value)} placeholder="Goals, audience, constraints, references, and the final outcome…" /></label>
      <div className="v3-brief-attach">
        <input ref={fileInput} type="file" multiple hidden onChange={onFilesChosen} />
        <button type="button" onClick={pickFiles} disabled={uploadingCount > 0}><Icon name="attach" />{uploadingCount > 0 ? `Uploading ${uploadingCount}…` : 'Attach reference files'}</button>
        {attachments.length > 0 && <ul>{attachments.map((file) => <li key={file.url}><Icon name="attach" size={13} /><a href={file.url} target="_blank" rel="noreferrer">{file.name}</a><button type="button" onClick={() => removeAttachment(file.url)} aria-label={`Remove ${file.name}`}><Icon name="close" size={12} /></button></li>)}</ul>}
      </div>
    </div></section>}

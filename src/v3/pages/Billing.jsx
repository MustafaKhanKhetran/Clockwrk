...
export default function Billing() {
  ...
  const active = requests.filter((item) => item.status === 'active' && !item.isParent).length;
  const activeRequests = requests.filter((item) => item.status === 'active' && !item.isParent);
  ...
  return <div className="v3-billing-page"><PageIntro index="Account ledger" title="Billing" copy="Your subscription, capacity, care coverage, and invoice records without the fine-print maze." />
    {nextDue && <section className={`v3-next-billing v3-enter${totalPending > 0 ? ' is-due' : ''}`}>
      ...
      {savingPerMonth > 0 && liveCadence === 'weekly' && <button onClick={() => setChangeTarget({ kind: 'cadence', target: 'monthly', label: 'monthly' })}>Save ${savingPerMonth.toLocaleString()}/mo by paying monthly</button>}
    </section>}
    ...
    <section className="v3-care-ledger"><header><div key={currentCare.id} className="v3-care-copy-fade"><span>Care included</span><h2>{currentCare.name} protects what ships.</h2><p>Monitoring, updates, fixes, and strategy stay connected to your build.</p></div><strong><AnimatedNumber prefix="$" value={currentCare.price} /><small> value / month</small></strong></header><div key={`${currentCare.id}-benefits`} className="v3-care-items-fade">{currentCare.includes.slice(0, 6).map((item) => <span key={item}><Icon name="check" size={14} />{item}</span>)}</div><button onClick={() => setCareOpen(true)}>Compare ongoing care <Icon name="arrow" size={15} /></button></section>

    <section className="v3-invoice-ledger v3-enter"><header><div><span>Documents</span><h2>Invoice history</h2></div><strong>{invoices.length} records</strong></header><div>{!invoices.length && <div className="v3-empty-panel v3-enter" style={{ marginTop: 12 }}>
        <Icon name="billing" size={26} />
        <strong>No invoices yet</strong>
        <span>Every confirmed payment lands here as a numbered invoice — INV-####, amount, date, plan.</span>
    ...
    {careOpen && (() => {
      const carePlanName = Object.fromEntries(Object.entries(PLAN_CARE).map(([planName, careId]) => [careId, planName]));
      return <div className="v3-dialog-layer" onMouseDown={() => setCareOpen(false)}>
        <section className="v3-care-compare" onMouseDown={(event) => event.stopPropagation()}>
          <header><span>Ongoing care</span><button onClick={() => setCareOpen(false)} aria-label="Close"><Icon name="close" size={16} /></button></header>
          <p>Every subscription includes ongoing care once work ships. Your care tier comes bundled with your plan — moving up a tier means moving up a plan.</p>
          <div className="v3-care-grid">
            {CARE_PLANS.map((tier) => {
              const isCurrent = tier.id === currentCare.id;
              const planFor = carePlanName[tier.id];
              return <article key={tier.id} className={isCurrent ? 'is-current' : ''}>
                <header>
                  <span>{isCurrent ? 'Your care' : planFor ? `With ${planFor}` : 'Care tier'}{tier.recommended && !isCurrent ? ' · Popular' : ''}</span>
                  <h3>{tier.name}</h3>
                  <strong>${tier.price.toLocaleString()}<small> value / mo</small></strong>
                </header>
                <dl>
                  <div><dt>Included hours</dt><dd>{tier.hoursIncluded} hrs / mo</dd></div>
                  <div><dt>Response time</dt><dd>{tier.responseTime}</dd></div>
                  <div><dt>Strategy call</dt><dd>{tier.strategyCall || 'Not included'}</dd></div>
                </dl>
                <ul>{tier.includes.map((line) => <li key={line}><Icon name="check" size={13} />{line}</li>)}</ul>
              </article>;
            })}
          </div>
          {accountMode !== 'retainer' && (
            <footer><span>Care follows your plan.</span><Action icon="arrow" onClick={() => { setCareOpen(false); setTiersOpen(true); }}>Change plan</Action></footer>
          )}
        </section>
      </div>;
    })()}

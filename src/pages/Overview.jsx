...
const PipelineCard = memo(function PipelineCard({ proposals, value }) {
  return (
    <section className="tw-card tw-compact-card tw-pipeline-card">
      <div className="tw-card-heading">
        <div>
          <span className="tw-kicker"><TrendingUp size={15} /> Pipeline</span>
          <h2>Pipeline</h2>
        </div>
      </div>
      <div className="tw-pipeline-stats">
        <div><strong>{fmtNum(proposals)}</strong><span>proposals out</span></div>
        <div><strong>{fmtCompactPKR(value)}</strong><span>potential value</span></div>
      </div>
    </section>
  );
});
...
export default function Overview() {
  ...
  const activeProposals = stats?.active_proposals ?? stats?.open_proposals ?? stats?.total_bookings ?? bookings.length;
  const pipelineValue = stats?.pipeline_value ?? stats?.potential_value ?? stats?.proposal_value ?? 0;
            ...
            <div className="tw-dashboard-grid">
              ...
              <div className="tw-right-column">
                <div className="tw-side-stack">
                  ...
                  />
                  <PipelineCard proposals={activeProposals} value={pipelineValue} />
